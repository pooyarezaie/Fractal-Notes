...
- `_includes/` holds five small partials: `series-nav.html` (course breadcrumb + prev/next), `note-authors.html` (resolves a page's `author`/`authors` keys against `_data/authors.yml`), `note-byline.html` (the «نوشتهٔ …» line after a note's body), `author-page.html` (body of an author page) and `fa-number.html` (integer → Persian-Indic digits).
- **Authors:** `_data/authors.yml` maps a key to `name` (Persian), optional `github` / `x` usernames, and an internal `twitter` for the card creator. `_config.yml` defaults every page's `author` to `pooyarezaie`, so only contributed notes set `author:` in front matter. Each author has a page at `authors/<key>.md` (`/authors/<key>/`) — name, links, and their notes grouped by topic, all derived from `site_index` + front matter; GitHub Pages can't run a generator plugin, so the page file itself is created by hand once. The byline is deliberately quiet — one small grey line at the end of the note linking to that page, never at the top; the content is the point, the credit follows it. The same resolved author feeds the layout's JSON-LD (`Article.author` with `url` = the author page and `sameAs` = the profiles; author pages emit a `ProfilePage`), jekyll-seo-tag's `<meta name="author">`/Twitter creator, and a per-entry `<author>` in `feed.xml`. The site footer states the reuse rule once (CC BY 4.0, name the author, link back).
