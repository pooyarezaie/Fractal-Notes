...
- `_includes/` holds four small partials: `series-nav.html` (course breadcrumb + prev/next), `note-authors.html` (resolves a page's `author`/`authors` keys against `_data/authors.yml`), `note-byline.html` (the «نوشتهٔ …» line after a note's body) and `fa-number.html` (integer → Persian-Indic digits).
- **Authors:** `_data/authors.yml` maps a key to `name` (Persian), optional `url`, and `twitter`. `_config.yml` defaults every page's `author` to `pooya`, so only contributed notes set `author:` in front matter. The byline is deliberately quiet — one small grey line at the end of the note, never at the top; the content is the point, the credit follows it. The same resolved author feeds the layout's JSON-LD `Article.author`, jekyll-seo-tag's `<meta name="author">`/Twitter creator, and a per-entry `<author>` in `feed.xml`. The site footer states the reuse rule once (CC BY 4.0, name the author, link back).
